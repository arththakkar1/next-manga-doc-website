...
export default function TagPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["search", "", [tagId], page],
    queryFn: async () => {
      const res = await fetch(
        `/api/manga/search?title=&tags=${tagId}&limit=${limit}&offset=${offset}`
      );
  ...
  const changePage = (newPage: number) => {
    ...
    router.push(`/tags/${tagId}/${tagName}?${params.toString()}`);
  };

  return (
    <main className="min-h-screen pt-28 w-full bg-black text-white">
      <h1 className="text-3xl font-bold px-5 mb-4">
        Manga with Tag:{" "}
